...
import type { CAPIResponse, Certificate, EimzoUserType } from "./types";

// Wsocket API для взаимодействия с E-IMZO CAPI
const CAPIWS = {
  URL:
    (window.location.protocol === "https:" ? "wss://127.0.0.1:64443" : "ws://127.0.0.1:64646") +
    "/service/cryptapi",
  ...
  callFunction: function (funcDef: object): Promise<{
    ...
    return new Promise((resolve, reject) => {
      if (!window.WebSocket) return reject("Browser doesn't support WebSocket");
      ...
      socket.onopen = () => socket.send(JSON.stringify(funcDef));
      ...
      socket.onclose = (e) => {
        if (e.code !== 1000) reject(`Socket closed with code ${e.code}`);
      };
    });
  },
};
...
export class EIMZOClient {
  ...
  async init() {
    ...
    if (!data.success) throw new Error(data.reason || "Unknown version error");

    const version = parseInt(data.major) * 100 + parseInt(data.minor);

    const { data: apiData } = await CAPIWS.callFunction({
      name: "apikey",
      arguments: this.apiKeys,
    });
    if (!apiData.success) throw new Error("Ошибка API_KEY: " + apiData.reason);
  ...
  async listAllUserKeys(): Promise<Certificate[]> {
    ...
    return data.certificates
      ...
      .filter((cert: any) => {
        const expiry = new Date(cert.validTo.replace(/\./g, "-"));
        return expiry > now;
      });
  }
  ...
  async listCkcTokens(): Promise<any[]> {
    ...
    if (!data.success) {
      throw new Error(data.reason || "Ошибка при получении списка токенов");
    }
  ...
  async createPkcs7(keyId: string, content: string, isBase64 = false): Promise<string> {
    const data64 = isBase64 ? content : EimzoBase64.encode(content);
    const { data } = await CAPIWS.callFunction({
      plugin: "pkcs7",
      name: "create_pkcs7",
  ...
  async createPkcs7ForToken(content: string, isBase64 = false): Promise<string> {
    const data64 = isBase64 ? content : EimzoBase64.encode(content);
    ...
    if (!data.success) {
      throw new Error(data.reason || "Ошибка при подписании токеном");
    }
